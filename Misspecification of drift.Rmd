...
To this end, we will simulate realizations from a model with double-well potential and use the likelihood of 3'rd degree polynomial potential model to estimate the tipping time and simulate the distribution of the estimator.
...
```
It turns out to be quite fast, so we can easily rerun if needed.
...
```
We inspect the distribution on the tau-estimates:
```{r}
hist(estimates_matrix[,4], breaks = 20)
mean(estimates_matrix[,4])
```
Making a ggplot
```{r}
tau <- estimates_matrix[,4]
plt.data <- data.frame(tau)
ggplot(data = plt.data, aes(x = tau)) + geom_histogram(bins = 50) + geom_vline(aes(xintercept = mean(tau)), color = "red") +geom_vline(aes(xintercept = 200), color = "blue")
```
