...
min_sig_thresh <- seq(from = 0.70, to = 0.95, length.out = 5)
no_gridpoints <- 10
t.obs <- seq(from = 1000, to = 3000, length.out = 5)
plots <- list(length = 5)
for (i in (1:5)){
  grid <- seq(from = min_sig_thresh[i], to = 1, length.out = no_gridpoints)
  plots[[i]] <- ROC.maker(H1.data = var.matrix.H1, H0.data = var.matrix.H0, grid = grid, mean0 = gam0, var0 = vargam0, t.obs = t.obs[i], dt = dt, Tw = Tw)
}
plots[1]
plots[2]
plots[3]
...
for (i in (1:5)){
  grid <- seq(from = min_sig_thresh[i], to = 1, length.out = no_gridpoints)
  plots[[i]] <- ROC.maker(H1.data = rho.matrix.H1, H0.data = rho.matrix.H0, grid = grid, mean0 = rho0, var0 = varrho0, t.obs = t.obs[i], dt = dt, Tw = Tw)
}
plots[1]
plots[2]
plots[3]
