...
for (i in (1:nsim)){
  ...
  var_gamma_0 <- 2*gamma2_0^2*(1+rho_0^2)/((1-rho_0)*1200)
  var_rho_0 <- (1-rho_0^2)/1200
  baseline_estimates_H1[,i] <- c(gamma2_0, rho_0, var_gamma_0, var_rho_0)
}
```
...
for(i in 1:(n - T_w_var/dt)){
  for(j in 1:rep){
    data.i = X.H1[i:(i+T_w_var/dt),j] ## Get data in running window
...
for(i in 1:(n - T_w_rho/dt)){
  for(j in 1:rep){
    data.i = X.H1[i:(i+T_w_rho/dt),j] ## Get data in running window
    data.i = data.i[data.i > -2.3] ## Remove data if tipped
    time.i = (1:length(data.i))*dt ## Get time interval for data in running window
    temp   = lm(data.i ~ time.i)   ## Get linear trend
    data.i = data.i - temp$fitted.values ## Subtract linear trend
...
var.matrix.H0 = matrix(-1, ncol = rep, nrow = n - T_w_var/dt)
for(i in 1:(n - T_w_var/dt)){
  for(j in 1:rep){
    data.i = X.H0[i:(i+T_w_var/dt),j] ## Get data in running window
    data.i = data.i[data.i > -2.3] ## Remove data if tipped
    time.i = (1:length(data.i))*dt ## Get time interval for data in running window
    temp   = lm(data.i ~ time.i)   ## Get linear trend
    data.i = data.i - temp$fitted.values ## Subtract linear trend
...
rho.matrix.H0 = matrix(-1, ncol = rep, nrow = n - T_w_rho/dt)
for(i in 1:(n - T_w_rho/dt)){
  for(j in 1:rep){
    data.i = X.H0[i:(i+T_w_rho/dt),j] ## Get data in running window
    data.i = data.i[data.i > -2.3] ## Remove data if tipped
    time.i = (1:length(data.i))*dt ## Get time interval for data in running window
    temp   = lm(data.i ~ time.i)   ## Get linear trend
    data.i = data.i - temp$fitted.values ## Subtract linear trend
...
save(var.matrix.H1, rho.matrix.H1, var.matrix.H0, rho.matrix.H0, file = "EWS.matrices.new.windows")
```
...
```{r}
ROC.maker <- function(H1.data, H0.data, grid, mean0_H1, var0_H1, mean0_H0, var0_H0, t.obs = 3000, dt = 0.1, Tw = 50){
  ...
  for (i in (1:no_thresh)){
    ...
    #browser()
    for(j in (1:no_paths)){
      thresh <- mean0_H1[j] + thresholds[i]*sqrt(var0_H1[j])
  ...
  for (i in (1:no_thresh)){
    no_triggered <- 0
    for(j in (1:no_paths)){
      thresh <- mean0_H0[j] + thresholds[i]*sqrt(var0_H0[j])
...
```{r}
mean0_H1_var <- baseline_estimates_H1[1,]
var0_H1_var <- baseline_estimates_H1[3,]
mean0_H0_var <- baseline_estimates_H0[1,]
var0_H0_var <- baseline_estimates_H0[3,]

times <- seq(from = 0, to = 1, length.out = 5)
min_sig_thresh <- seq(from = 0, to = 0.7, length.out = 5)
no_gridpoints <- 30
t.obs <- seq(from = ceiling(T_w_var)/dt, to = 3000, length.out = 5)
plots.var <- list(length = 5)
```
```{r}
grid_var_1 <- seq(from = 0, to = 1, length.out = 30)
grid_var_2 <- seq(from = 0.4, to = 1, length.out = 20)
grid_var_2 <- c(grid_var_2, seq(from = 0.99, to = 1, length.out = 5))
grid_var_3 <- seq(from = 0.5, to = 1, length.out = 20)
grid_var_3 <- c(grid_var_3, seq(from = 0.99, to = 1, length.out = 20))
grid_var_3 <- c(grid_var_3, seq(from = 0.999, to = 1, length.out = 5))
grid_var_4 <- seq(from = 0.7, to = 1, length.out = 20)
grid_var_4 <- c(grid_var_4, seq(from = 0.99, to = 1, length.out = 10))
grid_var_4 <- c(grid_var_4, seq(from = 0.9995, to = 1, length.out = 10))
grid_var_4 <- c(grid_var_4, seq(from = 0.99999, to = 1, length.out = 10))
grid_var_5 <- seq(from = 0.75, to = 1, length.out = 20)
grid_var_5 <- c(grid_var_5, seq(from = 0.99, to = 1, length.out = 10))
grid_var_5 <- c(grid_var_5, seq(from = 0.9999, to = 1, length.out = 10))
grid_var_5 <- c(grid_var_5, seq(from = 0.9999999, to = 1, length.out = 3))
grids_var <- list(grid_var_1, grid_var_2, grid_var_3, grid_var_4, grid_var_5)
for (i in (1:5)){
  #grid <- seq(from = min_sig_thresh[i], to = 1, length.out = no_gridpoints)
  #grid <- grid^(1/(5*i))
  plots.var[[i]] <- ROC.maker(H1.data = var.matrix.H1, H0.data = var.matrix.H0, grid = grids_var[[i]], mean0_H1 = mean0_H1_var, var0_H1 = var0_H1_var, mean0_H0 = mean0_H0_var, var0_H0 = var0_H0_var, t.obs = t.obs[i], dt = dt, Tw = T_w_var)
}

plt <- plot_grid(plotlist = plots.var, labels = c("a = 0", "a = 0.25", "a = 0.5", "a = 0.75", "a = 1"), label_x = 0.5, label_y = 0.35)
title <- ggdraw() + 
  draw_label(
    "ROC-curves for Variance EWS",
    fontface = 'bold',
    x = 0,
    hjust = 0
  ) +
...
```{r}
mean0_H1_rho <- baseline_estimates_H1[2,]
var0_H1_rho <- baseline_estimates_H1[4,]
mean0_H0_rho <- baseline_estimates_H0[2,]
var0_H0_rho <- baseline_estimates_H0[4,]

times <- seq(from = 0, to = 1, length.out = 5)
min_sig_thresh <- seq(from = 0, to = 0.7, length.out = 5)
no_gridpoints <- 30
t.obs <- seq(from = ceiling(T_w_rho)/dt, to = 3000, length.out = 5)
plots.rho <- list(length = 5)

grid_rho_1 <- seq(from = 0, to = 1, length.out = 30)
grid_rho_2 <- seq(from = 0.4, to = 1, length.out = 30)
#grid_rho_2 <- c(grid_rho_2, seq(from = 0.99, to = 1, length.out = 5))
grid_rho_3 <- seq(from = 0.5, to = 1, length.out = 20)
grid_rho_3 <- c(grid_rho_3, seq(from = 0.99, to = 1, length.out = 10))
#grid_rho_3 <- c(grid_rho_3, seq(from = 0.999, to = 1, length.out = 5))
grid_rho_4 <- seq(from = 0.7, to = 1, length.out = 20)
grid_rho_4 <- c(grid_rho_4, seq(from = 0.99, to = 1, length.out = 10))
grid_rho_4 <- c(grid_rho_4, seq(from = 0.9995, to = 1, length.out = 10))
grid_rho_5 <- seq(from = 0.75, to = 1, length.out = 20)
grid_rho_5 <- c(grid_rho_5, seq(from = 0.99, to = 1, length.out = 10))
grid_rho_5 <- c(grid_rho_5, seq(from = 0.9999, to = 1, length.out = 10))
grid_rho_5 <- c(grid_rho_5, seq(from = 0.9999999, to = 1, length.out = 10))
grids_rho <- list(grid_rho_1, grid_rho_2, grid_rho_3, grid_rho_4, grid_rho_5)

for (i in (1:5)){
  #grid <- seq(from = min_sig_thresh[i], to = 1, length.out = no_gridpoints)
  plots.rho[[i]] <- ROC.maker(H1.data = rho.matrix.H1, H0.data = rho.matrix.H0, grid = grids_rho[[i]], mean0_H1 = mean0_H1_rho, var0_H1 = var0_H1_rho, mean0_H0 = mean0_H0_rho, var0_H0 = var0_H0_rho, t.obs = t.obs[i], dt = dt, Tw = T_w_rho)
}

plt <- plot_grid(plotlist = plots.rho, labels = c("a = 0", "a = 0.25", "a = 0.5", "a = 0.75", "a = 1"), label_x = 0.5, label_y = 0.35)
title <- ggdraw() + 
  draw_label(
    "ROC-curves for Autocorrelation EWS",
    fontface = 'bold',
    x = 0,
    hjust = 0
  ) +
